...
def save_times(times_vector, labels, directory, components, window_size = None):
    ...
    if window_size == None:
        ...
        with open(f'{directory}/Simulation times for {components} components and sliding window length {window_size}.pkl', 'wb') as f:
            pickle.dump(times, f)
    return
def run_model (feature_extractor = None, classification_method = None, scaler = None):
    ...
    elif feature_extractor!=None:
        if feature_extractor not in ['mobilenet_small', 'resnet18']:
    ...
    fig_directory = f"figures/Original features/{feature_extractor}/{classification_method} model"
    if not os.path.exists(fig_directory):
        os.makedirs(fig_directory)
    else:
        print('Simulation already executed')
        return
    data_directory = f"saved_data/Original features/{feature_extractor}/{classification_method} model"
